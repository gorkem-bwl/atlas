...
// Cheap aggregate stats for the Invoices dashboard. Requires the
// connection to be 'connected' and the invoices "view" permission.
export async function getParasutDashboard(req: Request, res: Response) {
  try {
    const perm = req.invoicesPerm!;
    if (!canAccess(perm.role, 'view')) {
      res.status(403).json({ success: false, error: 'No permission to view invoices' });
    ...
    const data = await parasutService.getParasutDashboardStats(tenantId);
    res.json({ success: true, data });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to get Paraşüt dashboard stats';
    logger.error({ error }, 'Failed to get Paraşüt dashboard stats');
    res.status(400).json({ success: false, error: message });
  }
}
