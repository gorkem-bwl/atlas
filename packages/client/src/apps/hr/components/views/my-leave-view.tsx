...
export function MyLeaveView({ employees }: { employees: HrEmployee[] }) {
  ...
  // The HR submit endpoint returns a 400 with `{ error: "Insufficient
  // leave balance. Available: 0, Required: 3" }` on validation failures.
  // The friendly message lives in axios's `error.response.data.error`,
  // not `Error.message` (which is the generic "Request failed with
  // status code 400"). Same extraction pattern used across the codebase
  // (login, register, sign-public, org-members, calendar event-modal).
